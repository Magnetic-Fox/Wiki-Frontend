...
# Default known Wikis
knownWikis = {'Wikipedia PL': ['https://pl.wikipedia.org/',True,''],
...
def getWikiData(wikiURL, title, useStandardAPIUrl=True, nonStandardAPIUrl=""):
    ...
    url=wikiURL+APIUrl+"?action=query&prop=extracts&explaintext=true" \
                "&format=json&indexpageids=true&redirects=true&titles=" \
                +title
    try:
...
def pageMissing(wikiDataResponse):
    return ('-1' in listPages(wikiDataResponse)) and \
        (('missing' in \
        wikiDataResponse["query"]["pages"][listPages(wikiDataResponse)[0]]) or \
        ('invalid' in \
        wikiDataResponse["query"]["pages"][listPages(wikiDataResponse)[0]]))
...
def loadConfig():
    global knownWikis
    global wiki
    global textWrappingWidth
    global useStandardAPIUrl
    global nonStandardAPIUrl
        ...
        if len(title.lstrip())>0:
            ...
            # Gathering page and displaying results
                ...
                if useStandardAPIUrl:
                    ...
                    print("\n"+getFullPage(wiki,title.lstrip(),False, \
                            nonStandardAPIUrl)+"\n")
